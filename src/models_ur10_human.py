...
class Actor(nn.Module):
    def __init__(self, hidden_size, stochastic=True, layer_norm=False):
        ...
        layers = [nn.Linear(3, hidden_size), nn.Tanh(), nn.Linear(hidden_size, hidden_size), nn.Tanh(),
                  nn.Linear(hidden_size, 1)]
        if layer_norm:
            layers = layers[:1] + [nn.LayerNorm(hidden_size)] \
                     + layers[1:3] + [nn.LayerNorm(hidden_size)] \
                     + layers[3:]  # Insert layer normalisation between fully-connected layers and nonlinearities
...
class SoftActor(nn.Module):
    def __init__(self, hidden_size, state_size):
        ...
        layers = [nn.Linear(state_size, hidden_size), nn.Tanh(), nn.Linear(hidden_size, hidden_size), nn.Tanh(),
...
class Critic(nn.Module):
    def __init__(self, hidden_size, state_size, state_action=False, layer_norm=False):
        super(Critic, self).__init__()
        self.state_action = state_action
        layers = [nn.Linear(state_size + (1 if state_action else 0), hidden_size), nn.Tanh(),
                  nn.Linear(hidden_size, hidden_size), nn.Tanh(), nn.Linear(hidden_size, 1)]
        if layer_norm:
            layers = layers[:1] + [nn.LayerNorm(hidden_size)] + layers[1:3] + [nn.LayerNorm(hidden_size)] + layers[
                                                                                                            3:]  # Insert layer normalisation between fully-connected layers and nonlinearities
