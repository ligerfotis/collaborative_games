...
class Agent():
    def __init__(self, config=None, alpha=0.0003, beta=0.0003, input_dims=[8],
        ...
        self.critic_1 = CriticNetwork(self.beta, self.input_dims, n_actions=self.n_actions,
                                      name='critic_1', fc1_dims=self.layer1_size, fc2_dims=self.layer2_size,
                                      chkpt_dir=self.chkpt_dir)
        self.critic_2 = CriticNetwork(self.beta, self.input_dims, n_actions=self.n_actions,
                                      name='critic_2', fc1_dims=self.layer1_size, fc2_dims=self.layer2_size,
                                      chkpt_dir=self.chkpt_dir)
        self.value = ValueNetwork(self.beta, self.input_dims, name='value', fc1_dims=self.layer1_size,
                                  fc2_dims=self.layer2_size,
                                  chkpt_dir=self.chkpt_dir)
        self.target_value = ValueNetwork(self.beta, self.input_dims, name='target_value', fc1_dims=self.layer1_size,
                                         fc2_dims=self.layer2_size, chkpt_dir=self.chkpt_dir)
    ...
    def learn(self, episode=None):
        if episode is not None:  # online training
            state, action, reward, new_state, done = episode
