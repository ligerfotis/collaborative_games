...
from hyperparams_ur10 import OFF_POLICY_BATCH_SIZE as BATCH_SIZE, DISCOUNT, ENTROPY_WEIGHT, HIDDEN_SIZE, LEARNING_RATE, \
    MAX_STEPS, POLYAK_FACTOR, REPLAY_SIZE, UPDATE_INTERVAL, UPDATE_START, SAVE_INTERVAL, OFFLINE_UPDATES, TEST_NUM, \
    ACTION_DURATION
import rospkg
import sys
import os
...
def plot(time_elpsd, data, figure_title, y_axis_name, x_axis_name, path, save=True, variance=False, stdev=None,
         color='-ok', plt_type="simple"):
    """
    Plots a plt_type from the data with figure_title and y_axis_name-x_axis_name on the path
    - save option
    """
...
def plot_hist(data, path, title):
    """ Plots the histogram of 'data' in the 'path' directory with 'title'."""
    plt.figure()
    plt.title(title)
    plt.grid()
...
def subplot(plot_directory, turtle_pos, turtle_vel, turtle_acc, time_turtle_pos, time_turtle_vel, time_turtle_acc,
            real_act_list, time_real_act_list, axis, plot_type):
...
def main():
    ...
    if sys.argv[1] == "trials":
        ...
        plot(range(UPDATE_INTERVAL, MAX_STEPS + UPDATE_INTERVAL, UPDATE_INTERVAL), means_list, "trials", 'Tests Score',
             'Number of Interactions', plot_directory, save=True, variance=True, stdev=stdev_list)
    ...
    #         plot(time[1:], (actions[1:] - actions[:-1])/10, "Actions_Boxplot_diff", 'Actions Change', 'Timestamp', plot_directory, save=True, plt_type="log")
        ...
        for exp in range(exp_num):
            ...
            subplot(plot_directory, turtle_pos, turtle_vel, turtle_acc, time_turtle_pos, time_turtle_vel,
                    time_turtle_acc, real_act_list, time_real_act_list, axis, plot_type)
        ...
        human_action_time = np.genfromtxt(plot_directory + 'human_action_time' + plot_type + '.csv', delimiter=',')[
                            :1000]

        turtle_acc_x = np.genfromtxt(plot_directory + 'x_turtle_accel_' + plot_type + '.csv', delimiter=',')[:1000]
        time_turtle_acc_x = np.genfromtxt(plot_directory + 'x_turtle_accel_time_' + plot_type + '.csv', delimiter=',')[
                            :1000]
        ...
        plot(range(len(alpha_values)), alpha_values, "alpha_values", 'Alpha Value', 'Number of Gradient Updates',
             plot_directory, save=True)
        plot(range(len(policy_loss_list)), policy_loss_list, "policy_loss", 'Policy loss', 'Number of Gradient Updates',
             plot_directory, save=True)
        plot(range(len(value_loss_list)), value_loss_list, "value_loss_list", 'Value loss',
             'Number of Gradient Updates', plot_directory, save=True)
        plot(range(len(rewards_list)), rewards_list, "Rewards_per_game", 'Total Rewards per Game', 'Number of Games',
             plot_directory, save=True)
        plot(range(len(turn_list)), turn_list, "Steps_per_game", 'Turns per Game', 'Number of Games', plot_directory,
             save=True)

        plot(range(len(critics_lr_list)), critics_lr_list, "critics_lr_list", 'Critic lr', 'Number of Gradient Updates',
             plot_directory, save=True)
        plot(range(len(value_critic_lr_list)), value_critic_lr_list, "value_critic_lr_list", 'Value lr',
             'Number of Gradient Updates', plot_directory, save=True)
        plot(range(len(actor_lr_list)), actor_lr_list, "actor_lr_list", 'Actor lr', 'Number of Gradient Updates',
             plot_directory, save=True)

        plot(range(len(mean_list)), mean_list, "trials", 'Tests Score', 'Number of Interactions', plot_directory,
             save=True, variance=True, stdev=stdev_list)

    elif sys.argv[1] == "plot_actions":
    ...
    elif sys.argv[1] == "plot_actions_boxplot":
        path = sys.argv[2]
        ...
        plt.grid()
        plt.scatter(time_real_act_list[1:len(time_real_act_list) / 8], real_act_list[1:len(real_act_list) / 8],
                    color='b', label="real")
        plt.scatter(used_human_act_time_list[1:len(used_human_act_time_list) / 8],
                    used_human_act_list[1:len(used_human_act_list) / 8], color='r', label="passed")
